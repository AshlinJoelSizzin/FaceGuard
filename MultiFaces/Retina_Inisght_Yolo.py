...
#         lambda crop: preprocessor.enhance_small_face(crop),
        
#         # Strategy 2: Simple high-quality resize
#         lambda crop: cv2.resize(crop, (112, 112), interpolation=cv2.INTER_CUBIC),
        
#         # Strategy 3: Contrast enhancement only
#         lambda crop: cv2.convertScaleAbs(crop, alpha=1.2, beta=15),
        
#         # Strategy 4: CLAHE preprocessing
#         lambda crop: preprocessor.adaptive_histogram_equalization(crop)
#     ]
...
#                 processed_crop = cv2.resize(processed_crop, (112, 112))
...
# # --- FIXED RETINAFACE DETECTION ---
# def retinaface_detect_faces(image: np.ndarray):
#     """FIXED: RetinaFace with bbox expansion and robust embedding extraction"""
...
#                     break
#             except Exception as e:
#                 logger.warning(f"RetinaFace failed at threshold {threshold}: {e}")
#                 continue
...
#                 # CRITICAL FIX: Expand bbox for better context (20% padding)
#                 buffer_x = int((x2 - x1) * 0.2)
#                 buffer_y = int((y2 - y1) * 0.2)
#                 x1 = max(0, x1 - buffer_x)
#                 y1 = max(0, y1 - buffer_y)
#                 x2 = min(image.shape[1], x2 + buffer_x)
#                 y2 = min(image.shape[0], y2 + buffer_y)
                
#                 if x2 <= x1 or y2 <= y1:
#                     continue
                
#                 # Extract expanded face crop
...
#                 # CRITICAL FIX: Robust embedding extraction with multiple strategies
...
#                 else:
#                     logger.debug(f"RetinaFace {key}: Embedding extraction failed")
...
#     """Production-grade ensemble face detection with all optimizations"""
#     if image is None or image.size == 0:
#         return []
    
#     all_detections = []
    
#     # Primary detector: RetinaFace (highest accuracy)
...
# # --- FASTAPI APPLICATION ---
# app = FastAPI(title="Production Face Recognition System - Fixed", version="6.0")
...
# ):
#     """Production image upload with comprehensive face matching"""
#     conn = None
#     img_path = None
...
#             "average_quality": round(avg_quality, 3),
#             "system_version": "6.0 - RetinaFace Fixed"
#         }
...
#         },
#         "features": {
#             "retinaface_bbox_expansion": True,
...
#     # To run in the main thread, you would use this:
...
# async def send_notification_email(recipient_email: str, username: str):
#     """Sends a malicious upload notification email."""
    
#     subject = "Security Alert: Your Photo Was Detected in an Upload"
#     body = f"""
#     Hello {username},

#     This is a security alert from ImageAuth.

#     A recently uploaded image was found to contain a match with your registered face profile. 
#     This may indicate a malicious or unauthorized use of your photo. Please let us know if you want to Approve or Reject this action.

#     And we recomment you to please review your account activity.
...
#         logger.info(f"✅ Notification email sent to {username} at {recipient_email}")
#     except Exception as e:
...
#                 matches.append((user_id, sim))
#                 try:
#                     # --- START: MODIFIED CODE TO SEND NOTIFICATION ---
                    
#                     # 1. Fetch the matched user's email and username from the database
#                     user_info = await conn.fetchrow("SELECT username, email FROM users WHERE user_id=$1", user_id)
                    
#                     if user_info:
#                         # 2. Send the notification email
#                         await send_notification_email(user_info['email'], user_info['username'])
                    
#                     # --- END: MODIFIED CODE TO SEND NOTIFICATION ---
...
#                     logger.info(f"🔍 Recorded: user {user_id}, similarity {sim:.4f}")
#                 except Exception as e:
#                     logger.error(f"Detection recording or email sending failed for user {user_id}: {e}")
...
async def send_notification_email(
    ...
    *,
    ...
    """Sends a rich HTML security alert email that matches the branded layout."""

    # Fallbacks and formatting
    detected_at = detected_at or datetime.utcnow()
    similarity_pct = f"{(similarity or 0.0) * 100:.1f}%" if (similarity is not None and similarity <= 1.0) else f"{(similarity or 0.0):.1f}%"
    detected_user_id_text = detected_user_id if detected_user_id is not None else "—"
    uploader_ip_text = uploader_ip or "—"
    detected_time_text = detected_at.strftime("%Y-%m-%d %H:%M:%S UTC")

    subject = "Security Alert • ImageAuth Face Detection"

    # HTML email template (responsive, inline styling)
    html_body = f"""
    <!DOCTYPE html>
    <html lang="en">
    <head>
      ...
      <title>ImageAuth Security Alert</title>
    </head>
    <body style="margin:0;padding:0;background:#f5f7fb;font-family:Segoe UI,Roboto,Helvetica,Arial,sans-serif;color:#1f2937;">
      <table role="presentation" cellpadding="0" cellspacing="0" border="0" width="100%" style="background:#f5f7fb;padding:24px 0;">
        <tr>
          <td align="center">
            <table role="presentation" cellpadding="0" cellspacing="0" border="0" width="640" style="max-width:640px;width:100%;background:#ffffff;border-radius:12px;box-shadow:0 10px 30px rgba(31,41,55,0.08);overflow:hidden;">
              <!-- Header Banner -->
              <tr>
                <td style="padding:28px 24px;background:linear-gradient(135deg,#4f46e5 0%,#8b5cf6 50%,#ec4899 100%);color:#fff;text-align:center;">
                  <div style="font-size:14px;letter-spacing:.08em;opacity:.95;margin-bottom:6px;">Face Guard Security • Face Recognition System</div>
                  <div style="font-size:22px;font-weight:800;letter-spacing:.02em;">🔔 SECURITY ALERT</div>
                </td>
              </tr>
              ...
              <tr>
                <td style="padding:18px 24px 0 24px;">
                  <div style="font-size:16px;">Hello <strong>{username}</strong>,</div>
                  <div style="font-size:14px;color:#4b5563;margin-top:8px;">
                    A face detection event matched your registered profile. Please review the details below and take action if required.
                  </div>
                </td>
              </tr>
                  ...
                  <table role="presentation" cellpadding="0" cellspacing="0" border="0" width="100%" style="background:#f9fafb;border:1px solid #e5e7eb;border-radius:10px;">
                    <tr>
                      <td style="padding:14px 16px;border-bottom:1px solid #e5e7eb;">
                        <div style="font-weight:700;color:#374151;">🔎 Detection Details</div>
                      </td>
                    </tr>
                    <tr>
                      <td style="padding:12px 16px;">
                        <table role="presentation" cellpadding="0" cellspacing="0" border="0" width="100%" style="font-size:14px;color:#374151;">
                          <tr>
                            <td style="padding:8px 0;width:40%;color:#6b7280;">Detected User ID:</td>
                            <td style="padding:8px 0;"><strong>{detected_user_id_text}</strong></td>
                          </tr>
                          <tr>
                            <td style="padding:8px 0;width:40%;color:#6b7280;">Similarity Match:</td>
                            <td style="padding:8px 0;">
                              <span style="display:inline-block;background:#fef3c7;color:#b45309;border:1px solid #fde68a;border-radius:999px;padding:2px 10px;font-weight:700;">
                                {similarity_pct}
                              </span>
                            </td>
                          </tr>
                          <tr>
                            <td style="padding:8px 0;width:40%;color:#6b7280;">Uploader IP:</td>
                            <td style="padding:8px 0;"><code style="background:#f3f4f6;border:1px solid #e5e7eb;border-radius:6px;padding:2px 6px;">{uploader_ip_text}</code></td>
                          </tr>
                          <tr>
                            <td style="padding:8px 0;width:40%;color:#6b7280;">Detection Time:</td>
                            <td style="padding:8px 0;">{detected_time_text}</td>
                          </tr>
                        </table>
                      </td>
                    </tr>
              ...
              <tr>
                <td style="padding:16px 24px 0 24px;">
                  <table role="presentation" cellpadding="0" cellspacing="0" border="0" width="100%" style="background:#fffbeb;border:1px solid #fde68a;border-radius:10px;">
                    <tr>
                      <td style="padding:12px 16px;">
                        <div style="font-weight:700;color:#92400e;display:flex;align-items:center;">⚠️ Security Notice</div>
                        <div style="font-size:14px;color:#7c2d12;margin-top:6px;line-height:1.5;">
                          A registered user was detected in an uploaded image above your configured threshold. Please review and take appropriate action.
                        </div>
                      </td>
                    </tr>
                  </table>
              ...
              <!-- Action Buttons (optional routing to your app) -->
              <tr>
                <td style="padding:16px 24px 0 24px;">
                    ...
                    <tr>
                      <td align="left" style="padding:6px 0;">
                        <a href="#" style="text-decoration:none;">
                          <span style="background:#10b981;color:#ffffff;border-radius:8px;padding:10px 14px;font-weight:700;font-size:14px;display:inline-block;">Approve</span>
                        </a>
                        <a href="#" style="text-decoration:none;margin-left:8px;">
                          <span style="background:#ef4444;color:#ffffff;border-radius:8px;padding:10px 14px;font-weight:700;font-size:14px;display:inline-block;">Deny</span>
                        </a>
                      </td>
                    </tr>
                  </table>
              ...
              <tr>
                <td style="padding:16px 24px 0 24px;">
                  <table role="presentation" cellpadding="0" cellspacing="0" border="0" width="100%" style="background:#ecfdf5;border:1px solid #a7f3d0;border-radius:10px;">
                    <tr>
                      <td style="padding:12px 16px;">
                        <div style="font-weight:700;color:#065f46;display:flex;align-items:center;">✅ System Status</div>
                        <div style="font-size:14px;color:#065f46;margin-top:6px;line-height:1.5;">
                          ImageAuth is operating normally. This alert was generated in real time upon detection.
                        </div>
                      </td>
                    </tr>
                  </table>
              ...
              <!-- Footer -->
              <tr>
                <td style="padding:18px 24px 24px 24px;color:#6b7280;font-size:12px;text-align:center;">
                  ImageAuth Security • Automated Session Monitoring<br/>
                ...
                try:
                    ...
                    if user_info:
                        ...
                        await send_notification_email(
                            ...
                            detected_user_id=user_id,    # Detected user's actual ID
                            similarity=sim,                # Actual similarity e.g. 0.41 for 41.0%
                            uploader_ip=ip,              # Real uploader IP
                            detected_at=datetime.utcnow(),       # Detection timestamp
                            # detected_username=detected_user_info['username'] if detected_user_info else "Unknown"
)
