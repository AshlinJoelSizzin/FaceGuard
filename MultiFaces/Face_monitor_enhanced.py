...
def preprocess_image(image):
    ...
    try:
        ...
        # Sharpening for better feature extraction
...
def assess_face_quality(face_crop, bbox):
    """Assess face quality before processing"""
    try:
        x1, y1, x2, y2 = bbox
        face_area = (x2 - x1) * (y2 - y1)
        
        # Minimum face size threshold (50x50 pixels)
        if face_area < 2500:
        ...
        if blur_score < 100:
            return False, "Too blurry", blur_score / 1000.0
        
        # Brightness check
        mean_brightness = np.mean(gray)
        if mean_brightness < 50 or mean_brightness > 200:
            return False, "Poor lighting", 0.3
        
        # Calculate quality score (0-1)
        area_score = min(face_area / 10000.0, 1.0)  # Normalize by 100x100
        blur_quality = min(blur_score / 500.0, 1.0)  # Normalize blur score
        brightness_quality = 1.0 - abs(mean_brightness - 125) / 125.0
        
        quality_score = (area_score + blur_quality + brightness_quality) / 3.0
        
        return True, "Good quality", quality_score
        
    except Exception as e:
        logger.error(f"Quality assessment failed: {e}")
        return False, "Assessment failed", 0.0

def calculate_quality_score(face_crop):
    ...
    try:
        ...
        contrast_score = min(float(contrast / 60.0), 1.0)
        
        # Brightness distribution
        hist = cv2.calcHist([gray], [0], None, [256], [0, 256])
        brightness_score = 1.0 - np.sum(hist[:50]) / np.sum(hist) - np.sum(hist[200:]) / np.sum(hist)
        brightness_score = max(0.0, float(brightness_score))
        
        return (sharpness_score + contrast_score + brightness_score) / 3.0
...
def deduplicate_by_quality(detections):
    """Remove duplicate detections keeping highest quality ones"""
    if len(detections) <= 1:
        return detections
    
    # Sort by quality score (highest first)
    detections.sort(key=lambda x: x.get('quality_score', 0), reverse=True)
    ...
    for detection in detections:
        ...
        for existing in filtered:
            ...
            if iou_bbox(bbox_a, bbox_b) > 0.5:  # High overlap threshold
                is_duplicate = True
                break
...
def enhanced_ensemble_detect_faces(image: np.ndarray):
    """Enhanced multi-scale face detection with quality filtering"""
    ...
    # Multiple scale processing for better coverage
    scales = [1.0, 1.2, 0.8]  # Original, upscaled, downscaled
    
    for scale in scales:
        try:
            if scale != 1.0:
                h, w = processed_image.shape[:2]
                scaled_img = cv2.resize(processed_image, (int(w * scale), int(h * scale)))
            else:
                scaled_img = processed_image.copy()
            
            # Run original ensemble detection on scaled image
            scale_detections = ensemble_detect_faces_base(scaled_img)
            
            # Adjust bounding boxes back to original scale and assess quality
            for det in scale_detections:
                if scale != 1.0:
                    x1, y1, x2, y2 = det["bbox"]
                    det["bbox"] = (int(x1/scale), int(y1/scale), int(x2/scale), int(y2/scale))
                
                # Quality assessment on original image
                x1, y1, x2, y2 = det["bbox"]
                ...
                if x2 > x1 and y2 > y1:
                    ...
                    is_good, reason, quality_score = assess_face_quality(face_crop, (x1, y1, x2, y2))
                    
                    if is_good:
                        det["quality_score"] = quality_score
                        det["quality_reason"] = reason
                        all_detections.append(det)
                    else:
                        logger.debug(f"Face rejected: {reason}, quality: {quality_score:.2f}")
        
        except Exception as e:
            logger.error(f"Multi-scale detection failed for scale {scale}: {e}")
            continue
    
    # Remove duplicates and keep highest quality
    final_detections = deduplicate_by_quality(all_detections)
    
    logger.info(f"Enhanced detection: {len(all_detections)} -> {len(final_detections)} after quality filtering")
    
    return final_detections

def ensemble_detect_faces_base(image: np.ndarray):
    """Base ensemble detection (original function)"""
    if image is None or image.size == 0:
        return []
...
# Use enhanced detection as default
def ensemble_detect_faces(image: np.ndarray):
    """Main face detection function - uses enhanced version"""
    return enhanced_ensemble_detect_faces(image)
...
def enhanced_cosine_similarity(a, b):
    ...
    try:
        ...
        logger.error(f"Enhanced cosine similarity calculation failed: {e}")
        return 0.0

def adaptive_similarity_threshold(face_quality_score, crowd_density):
    """Calculate adaptive similarity threshold based on conditions"""
    base_threshold = 0.55  # Lowered base threshold for better detection
    ...
    if crowd_density > 5:  # More than 5 faces detected
        crowd_penalty = -0.03
    ...
    final_threshold = base_threshold + quality_bonus + crowd_penalty
    return max(0.4, min(0.75, final_threshold))  # Keep within reasonable bounds

# Keep original cosine_sim for backward compatibility
def cosine_sim(a, b):
    """Original cosine similarity function - enhanced version"""
    return enhanced_cosine_similarity(a, b)
...
async def register_user(
    ...
    try:
        ...
        # Detect faces using enhanced ensemble method
        faces_found = ensemble_detect_faces(img)

        if not faces_found:
            raise HTTPException(400, "No face detected in image.")

        logger.info(f"Found {len(faces_found)} faces with quality scores")
        ...
        for i, face in enumerate(faces_found):
            try:
                ...
                logger.info(f"Face {i}: quality={quality_score:.3f}, embedding type={type(base_embedding)}")
                ...
                if base_embedding is not None:
                    ...
                    if base_emb_list and len(base_emb_list) > 0:
                        ...
                        # Add augmented versions for high-quality faces
                        aligned_face = face.get("aligned")
                        if aligned_face is not None and quality_score > 0.6:
                            try:
                                # Generate fewer augmented versions for efficiency
                                aug_faces = augment_face(aligned_face, n=2)
...
async def upload_image(
    ...
    """Enhanced upload and analyze image for face detection"""
    conn = None
    img_path = None
    ...
    try:
        ...
        for i, det in enumerate(detections):
            ...
            quality = det.get("quality_score", 0.0)
            logger.info(f"Upload face {i}: embedding length={len(emb)}, quality={quality:.3f}")
        ...
        # Enhanced face comparison with adaptive thresholds
        profiles = await conn.fetch("SELECT user_id, embeddings FROM user_face_profiles")
        matches = []
        user_matches = {}
        crowd_density = len(detections)

        logger.info(f"Comparing against {len(profiles)} registered profiles with {crowd_density} faces detected")
        ...
        for det in detections:
            ...
            det_emb_np = np.array(det_emb)
            face_quality = det.get("quality_score", 0.5)
            
            # Calculate adaptive threshold
            threshold = adaptive_similarity_threshold(face_quality, crowd_density)
            logger.info(f"Using adaptive threshold: {threshold:.3f} (quality={face_quality:.3f}, crowd={crowd_density})")
            ...
            for profile in profiles:
                ...
                logger.debug(f"User {profile['user_id']} has {len(profile_embeddings)} stored embeddings")

                # Find best similarity for this user with enhanced calculation
                best_sim = 0
                for ref_emb in profile_embeddings:
                    if not ref_emb:
                        continue

                    sim = enhanced_cosine_similarity(det_emb_np, ref_emb)
                    logger.debug(f"User {profile['user_id']} similarity: {sim:.4f} (threshold: {threshold:.3f})")
        ...
        return {
            ...
            "avg_quality": sum(det.get("quality_score", 0) for det in detections) / len(detections)
        }
